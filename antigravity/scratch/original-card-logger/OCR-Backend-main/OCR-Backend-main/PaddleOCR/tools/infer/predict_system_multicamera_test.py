...
import random
import re
from helpers import get_cropped_frame
...
def insert_data_into_drivers_table(time, name, cnic, img):
    try:
        # Connect to the PostgreSQL database
        conn = psycopg2.connect(
            dbname="anpr",
        ...
        cursor = conn.cursor()
        sql = "INSERT INTO driver_ids (time, name, cnic, image_data) VALUES (%s, %s, %s, %s)"
        cursor.execute(sql, (time, name, cnic, img))
        conn.commit()
    except (Exception, psycopg2.Error) as error:
        print("Error while connecting to PostgreSQL:", error)
    finally:
...
class CustomProvider(BaseProvider):
    def pakistani_name(self):
        # List of common Pakistani names
        names = ["Ali", "Fatima", "Ahmed", "Ayesha", "Imran", "Zainab", "Omar", "Sana", "Bilal", "Hina"]
        return random.choice(names)
    
    def pakistani_cnic(self):
        # Generate a random CNIC in the format: XXXXX-XXXXXXX-X
...
def main(args, vid_path):
    ...
    global change_timer
    change_timer = 0
    change_threshold = 5  # 5 seconds threshold for consecutive change

    def detect_change(frame, prev_gray, img_list, threshold=1000):
        global change_timer
        ...
        if prev_gray is not None:
            if prev_gray.shape == gray.shape:
                ...
                if non_zero_count > threshold:
                    # Increment the change timer
                    change_timer += 1

                    # Check if the change duration exceeds the threshold
                    if change_timer >= change_threshold:
                       # print("Change detected for more than 5 seconds!")
                        change_timer = 0  # Reset the timer
                        if len(img_list) == 1:
                            ...
                            for k, (boxes, res) in enumerate(zip(dt_boxes, rec_res)):
                                if len(res) > 0:
                                    ...
                                    cnic_found = False
                                    for item, conf in res:
                                        if isinstance(item, str) and re.match(pattern, item):
                                            cnic = item
                                            cnic_found = True

                                    if not cnic_found:
                                        cnic = fake.pakistani_cnic()

                                    random_name = fake.pakistani_name()
                                    encoded_imgg = encode_image(img_list[0])
                                    insert_data_into_drivers_table(timestamp, str(random_name), str(cnic), str(encoded_imgg))
                                    print('CARD: ', timestamp, " ", res)
                        # Clear the image list
                        img_list = []
                else:
                    change_timer = 0  # Reset the timer if no change detected
                    # img_list = []
    ...
    # cap = cv2.VideoCapture(vid_path)
        ...
        cap2 = cv2.VideoCapture('rtsp://admin:admin123@10.1.15.240')
    elif lane == 'rfid2':
...
# Commented to save some Space
    ...
    while True:
        ...
        if frame_count % skip == 0:
            if not ret:
                print("Error: Can't receive frames (stream end?). Exiting ...")
                break
            starttime = time.time()
            # for 62 stream
            if lane == 'rfid':
            ...
            img2 = get_cropped_frame(img2)  

            img_list.append(img)
            img_list2.append(img2)
            # timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            if prev_frame2 is not None:
                detect_change(img2, prev_gray2, img_list2)
            print ("checking cap 2")
            if ret2 is not None:
                print(text_sys(img_list2))
            else:
                print("cap2 returns null")
            # Update previous frames and previous grayscale frame for camera 2
