...
const CameraEditForm: React.FC<{ 
  ...
  // Extracting the protocol, ip, username and password from the camera URL
  const urlParts = cam_url.split("://");
  const selectedProtocol = urlParts[0];
  const urlParts2 = urlParts[1].split(":");
  const selectedUsername = urlParts2[0];
  const urlParts3 = urlParts2[1].split("@");
  const selectedPassword = urlParts3[0];
  const selectedIP = urlParts3.slice(1).join("@").split("/")[0];
