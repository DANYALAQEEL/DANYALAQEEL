...
def run_system(
    ...
    for cam in cams:
        # Skip cameras explicitly disabled (cam_url == "99" is our sentinel value)
        if str(cam.get("cam_url", "")).strip() == "99":
            print(f"Skipping disabled camera id={cam['id']} name={cam['name']}")
            continue
