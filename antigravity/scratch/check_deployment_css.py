...
url = "https://ems-dashboard-final-5hlry4qf0-danyalaqeels-projects.vercel.app"
try:
    with urllib.request.urlopen(url) as response:
        html = response.read().decode('utf-8')
    
    print("Deployment page fetched. HTML length:", len(html))
    
    # Find stylesheet hrefs using regex
    css_urls = re.findall(r'<link[^>]*rel="stylesheet"[^>]*href="([^"]+)"', html)
    print("Deployment stylesheet links:", css_urls)
    ...
    for href in css_urls:
        ...
        print(f"Matches for FEFEF8: {len(fefef8_matches)}")
        if fefef8_matches:
            print("First match:", fefef8_matches[0])
