...
export default function OrgScheduleTasks() {
  ...
  const columns = [
    ...
    { key: 'device',   label: 'Device', render: v => <span className="text-xs text-surface-300">{v}</span> },
      ...
      <DataTable
        ...
        actions={(row) => (
          <>
            ...
            <button
              className={`btn-ghost p-1.5 text-success-400 ${running === row.id ? 'opacity-50 cursor-wait' : ''}`}
