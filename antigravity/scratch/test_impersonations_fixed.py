...
async def main():
    async with async_playwright() as p:
        ...
        # 1. Impersonate Org Admin (Role = Admin)
        print("\n--- TESTING ORG ADMIN IMPERSONATION ---")
        target_admin = None
        for u in user_list:
            if u["role"].lower() == "admin" and u["status"].lower() == "active" and u["has_btn"]:
                target_admin = u
                break
                
        if target_admin:
            print(f"Found active Org Admin: Name='{target_admin['name']}', Org='{target_admin['org']}', Email='{target_admin['email']}'")
            # Relocate row and click button
            row_el = (await page.query_selector_all("table tbody tr"))[target_admin["index"]]
            ...
            if btn_el:
                ...
                try:
                    ...
                    sb_links = await new_page.query_selector_all(".sidebar a, aside a, .kt-menu__link, .menu-item a")
                    print(f"Org Dashboard Sidebar has {len(sb_links)} links:")
                    printed = set()
                    for lnk in sb_links:
            ...
            print("No active Org Admin user with a login button was found!")
        ...
        if target_customer:
            ...
            if btn_el:
                ...
                try:
                    ...
                    # Extract sidebar items
                    sb_links = await new_page.query_selector_all(".sidebar a, aside a, .kt-menu__link, .menu-item a")
                    print(f"User Dashboard Sidebar has {len(sb_links)} links:")
                    printed = set()
                    for lnk in sb_links:
                        txt = (await lnk.inner_text()).strip().replace("\n", " ")
                        href = await lnk.get_attribute("href") or ""
