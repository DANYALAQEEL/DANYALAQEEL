...
def sync():
    ...
    print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting Antigravity-Blackbox Auto-Sync...")
    ...
    # 4. Stage monitored tracking paths
    items_to_add = [".gitignore", "GEMINI.md", "config/", "antigravity/mcp/", "antigravity/scratch/"]
    ...
    if not status_res.stdout.strip():
        print("No new changes detected. Repository is already up to date.")
    else:
    ...
    if push_res.returncode == 0:
        print("✅ Antigravity-Blackbox auto-sync completed successfully!")
    else:
